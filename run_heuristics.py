...
if __name__ == "__main__":
    ...
    if args.heuristics in ["SGM","Hikage","RSGM"]:
        ...
        with open("Dump/"+experiment_title+"/" + args.heuristics + "_Heuristics_ACTIONLIST","w",encoding="utf-8",newline="\n") as write_file:
            for action in action_group:
                write_file.write(str(action[0]) + "\t" + " ".join(map(str,action[1])) + "\n")
        with open("Dump/"+experiment_title+"/" + args.heuristics + "_Heuristics_RULE","w",encoding="utf-8",newline="\n") as write_file:
